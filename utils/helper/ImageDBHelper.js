			...
			function(status, response) {
				...
				mkdirp(fileFolderDest,
					function(err) {
						...
						fs.rename(fileFolderSource + '/' + file.filename, fileFolderDest + '/' + portfolioId + extension,
							function(err) {
								if(err) {
									return callback(500, err);
								}

								var image = credentials.host + dependenciesManager.imagesFolder + '/' + portfolioId + dependenciesManager.mainImageFolder + '/' + portfolioId + extension;
